  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">總營收</p><p className="text-2xl font-bold text-primary mt-1">${totalRevenue.toLocaleString()}</p></CardContent></Card>
        <Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">總進貨成本</p><p className="text-2xl font-bold text-accent mt-1">${totalPurchases.toLocaleString()}</p></CardContent></Card>
        <Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">零用金支出</p><p className="text-2xl font-bold text-destructive mt-1">${totalPettyCash.toLocaleString()}</p></CardContent></Card>
      </div>
      {!hasData ? (
        <Card><CardContent className="py-16 text-center text-muted-foreground"><p className="text-lg font-medium">尚無資料</p><p className="text-sm mt-1">請先在各模組中新增資料，報表將自動生成</p></CardContent></Card>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {monthlyRevenue.length > 0 && (
            <Card><CardHeader><CardTitle className="text-base">每月營收趨勢</CardTitle></CardHeader><CardContent><div className="h-64">
              <ResponsiveContainer width="100%" height="100%"><BarChart data={monthlyRevenue}><CartesianGrid strokeDasharray="3 3" stroke="hsl(35,15%,88%)" /><XAxis dataKey="month" tick={{ fontSize: 12 }} /><YAxis tick={{ fontSize: 12 }} /><Tooltip /><Bar dataKey="revenue" name="營收" fill="hsl(25,60%,42%)" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer>
            </div></CardContent></Card>
          )}
          {monthlyPurchases.length > 0 && (
            <Card><CardHeader><CardTitle className="text-base">每月進貨成本</CardTitle></CardHeader><CardContent><div className="h-64">
